...
#define PCIE_BAR_LOOP_TEST          _IO(IOC_MAGIC, 5)
...
#else
#define PCI_VENDOR_PF_ID_DEMO      0x1EC8
#define PCI_DEVICE_PF_ID_DEMO      0x1EB8
#define PCI_VENDOR_VF_ID_DEMO      0x1EC8
#define PCI_DEVICE_VF_ID_DEMO      0x9232
#endif
...
static long pcie_demo_ioctl (struct file * filp, unsigned int cmd, unsigned long args)
{
    ...
    switch(cmd){
        ...
        case PCIE_DEMO_READ_BAR:
            ...
            if(pbar->virt_addr == NULL) {	// need to ioremap
                pr_err("Is not a IO/MEMORY BAR");
                return -EINVAL;
            }
            tmpbuf = kmalloc(opt->len, GFP_KERNEL);	// PAGE_SIZE get_free_page()
            if(!tmpbuf){
                pr_err("short of memory");
                return -ENOMEM;
            }
