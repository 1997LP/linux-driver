...
struct test_info {
	struct dma_buf *dmabuf[DBUF_MAX_COUNT];
};
...
static int importer_delay_test(struct dma_buf *dmabuf)
{
	...
	vaddr = dma_buf_kmap(dmabuf, 0);	// called : test_dmabuf_ops_kmap
	pr_err("read from dmabuf kmap: %s\n", (char *)vaddr);
	dma_buf_kunmap(dmabuf, 0, vaddr);	// called : dmabuf->ops->unmap 可选实现
...
static long misc_demo_ioctl(struct file * filp, unsigned int cmd, unsigned long args)
{
	...
	case SET_DMABUF_FD: // dmabuf-importer-1 : 根据用户态fd，获取dma-buffer
	{
		...
		if (copy_from_user(dfd, (void __user *)args, sizeof(int)*DBUF_MAX_COUNT) != 0)
			return -EFAULT;
		g_tinfo.dmabuf[0] = dma_buf_get(dfd[0]);
		g_tinfo.dmabuf[1] = dma_buf_get(dfd[1]);
		g_tinfo.dmabuf[2] = dma_buf_get(dfd[2]);
		pr_err("import dmabuf is  %p-%p-%p\n", g_tinfo.dmabuf[0],g_tinfo.dmabuf[1],g_tinfo.dmabuf[2]);
	}
	break;
	case TEST_DMABUF_RW: // dmabuf-import-2 : 测试dma-buffer的读写
	{
		...
		importer_test(g_tinfo.dmabuf[index]);
	}
	break;
	case UNSET_DMABUF_FD : //dmabuf-import-3 : 释放dma-buffer
	{
		if(g_tinfo.dmabuf[0])
			dma_buf_put(g_tinfo.dmabuf[0]);
		if(g_tinfo.dmabuf[1])
			dma_buf_put(g_tinfo.dmabuf[1]);
		if(g_tinfo.dmabuf[2])
			dma_buf_put(g_tinfo.dmabuf[2]);
	}
	break;
	case TEST_DMABUF_USED:
	{
		...
		importer_delay_test(g_tinfo.dmabuf[index]);
	}
	break;
	default:
		break;
