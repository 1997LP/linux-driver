...
int main(int argc, char **argv)
{
	...
	res = drmModeGetResources(fd);//申请资源,fb ,crtc,encoder,connect
	crtc_id = res->crtcs[0];
	conn_id = res->connectors[0];

	drmSetClientCap(fd, DRM_CLIENT_CAP_UNIVERSAL_PLANES, 1);//DRM PLANE支持所有的资源
	plane_res = drmModeGetPlaneResources(fd); // plane resource
	...
	modeset_create_fb(fd, &buf);//create frame buffer

	drmSetClientCap(fd, DRM_CLIENT_CAP_ATOMIC, 1);//告诉驱动该程序支持atomic操作

	props = drmModeObjectGetProperties(fd, conn_id,	DRM_MODE_OBJECT_CONNECTOR);//获取connect的object id
	property_crtc_id = get_property_id(fd, props, "CRTC_ID");//获取connect property属性CRTC_ID
	drmModeFreeObjectProperties(props);

	props = drmModeObjectGetProperties(fd, crtc_id, DRM_MODE_OBJECT_CRTC);//获取crtc的object id
	property_active = get_property_id(fd, props, "ACTIVE"); //获取crtc的使能状态
	property_mode_id = get_property_id(fd, props, "MODE_ID");//获取crtc当前所使用的display mode id
	drmModeFreeObjectProperties(props);

	drmModeCreatePropertyBlob(fd, &conn->modes[0],
				sizeof(conn->modes[0]), &blob_id);

	req = drmModeAtomicAlloc();//分配atomic操作接口
	drmModeAtomicAddProperty(req, crtc_id, property_active, 1);//修改property的值
	...
	drmModeAtomicCommit(fd, req, DRM_MODE_ATOMIC_ALLOW_MODESET, NULL);//提交修改
